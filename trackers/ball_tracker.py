    ...
    def interpolate_ball_positions(self, ball_positions):
        ...
        for x in ball_positions:
            ...
            if bbox and len(bbox) == 4:
                ...
                # Append NaNs for missing or invalid bounding boxes
                processed_positions.append([np.nan, np.nan, np.nan, np.nan])

        # Convert to DataFrame for interpolation
        df = pd.DataFrame(processed_positions, columns=['x1', 'y1', 'x2', 'y2'])

        # Interpolate and fill missing values
        df = df.interpolate(limit_direction='both')
        df = df.bfill().ffill()  # in case the start or end are still NaN

        # Convert back to original format
        interpolated_positions = [{1: {"bbox": row}} for row in df.to_numpy().tolist()]
